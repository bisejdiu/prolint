...
from prolint.utils.metrics import FUNCS, calculate_metric, residence_time
...
class ComputeContacts(object):
    ...
    def _compute_contact(self, topology, mdtraj_contacts, PLASMA_LIPIDS, frames, timestep, residue='residue'):
                    ...
                    for r, idxs in enumerate(residue_indices):
                        ...
                        if not leightweight:
                            per_residue_results[chain_residue] = LPContacts(t.topology, md_out, PLASMA_LIPIDS,
                                                                    self.frames, self.timestep, residues[r])
                        else:
                            per_residue_results[chain_residue] = self._compute_contact(t.topology, md_out, PLASMA_LIPIDS,
                                                                    self.frames, self.timestep, mdtraj, residues[r])
...
class LPContacts(object):
    ...
    def __str__(self):
        return "<prolint.LPContacts for residue {}>".format(self.residue)
...
def retrieve_contacts(n, r, protein=None, contacts='contacts'):
    ...
    if (len(n.keys()) != 1) & (protein is None):
        raise ValueError("""prolint.Contacts provided contains information for more than 1 protein. \
...
#             raise ValueError("""prolint.Contacts provided contains information for more than 1 lipid. \
...
def contacts_dataframe(n, p, t, radius, co=0, custom_metrics=None, time_unit='us', rt=False, range_type='geo', norm=True):
            ...
            for k, v in metrics.items():
                ...
                if k != 'rt':
                    if (k in ['mean', 'max', 'sum']) & (norm):
                        result = calculate_metric(con, FUNCS[k], co, t, norm=norm)
                    else:
    ...
    return pd.DataFrame(RESULTS).sort_values(['Lipids', 'ResID']).reset_index(drop=True)
