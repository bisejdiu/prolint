...
class Lipids(SystemTopology):
    ...
    def __init__(self, topology, resolution="martini", lipid_names=None, atom_names=None):
        ...
        # try:
        #     if lipid_names and atom_names:
        #         user_indices = [atom.index for atom in topology.atoms
        #                         if ((atom.residue.name in lipid_names)
        #                         and (atom.name in atom_names))]
        #     elif lipid_names:
        #         user_indices = [atom.index for atom in topology.atoms
        #                         if (atom.residue.name in lipid_names)]
        #     elif atom_names:
        ...
        #     else:
        #         user_indices = self.ldf.index.to_numpy()

        # except (AttributeError, TypeError):
        #     raise AssertionError('lipid_names and atom_names input arguments must be list/array.')
    ...
    def __str__(self):
        return "<prolint.Lipids containing {} atom(s)>".format(self.l_indices.size)
    ...
    def system_proteins(self):
        ...
        for values in self.fi_li:
            ...
            if self.resolution == "martini":
                curr_len = len(current_protein_df[current_protein_df.name == "BB"])
            elif self.resolution == "atomistic":
                curr_len = len(current_protein_df[current_protein_df.name == "CA"])
